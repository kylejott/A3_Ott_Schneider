...
After having conducted first descriptive as well as inferential statistics it became quite clear that a better model is needed. Especially the attempt to analyze the top income-tax share dependent on the tax rate by running an ordinary least squares model has failed so far. Obviously controlling for GDP and considering year dummies didn’t lead to the expected outcome: We’re still considering to incorporate other data to build a worthy model. 
